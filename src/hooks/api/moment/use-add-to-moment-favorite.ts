import { addToMomentFavorite, type Moment } from "@/api/moment";
import useToast from "@/hooks/use-toast";
import { useMutation, useQueryClient } from "@tanstack/react-query";

export const useAddToMomentFavorite = (markerId: number, momentId: number) => {
  const { toast } = useToast();
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (id: number) => addToMomentFavorite(id),
    onMutate: async () => {
      ...
      if (selectedMomentIndex >= 0) {
        copy[selectedMomentIndex].userLiked = true;
        copy[selectedMomentIndex].thumbsUp =
          (copy[selectedMomentIndex]?.thumbsUp || 0) + 1;
      }
