...
const LocalChatList = () => {
  const { show, setTitle } = useChatStore();

  const handleClick = (code: string, location: string) => {
    const cid = localStorage.getItem("cid");
    if (!cid) return;
    setTitle(`${location} 채팅방`);
    show({ cid: JSON.parse(cid).cid, code });
  };
    ...
    <HorizontalScroll className="gap-4 py-1 px-1">
      {chatRoomData.map((v) => {
        return (
          <button
            key={v.location}
            onClick={() => handleClick(v.code, v.location)}
